...
class CircuitContext:
    ...
    def pragma_first_step(
        self: CircuitContext, step_type: StepTypeHandler
    ) -> None:  # TODO: define and use ToStepTypeHandler instead
...
class StepTypeContext:
    ...
    ) -> StepTypeWGHandler:  # Any instead of Args
        return StepTypeWGHandler(self.step_type.id, self.step_type.name, wg_def)
...
class Constraint:
    ...
    def if_next_step(
        step_type: StepTypeHandler, constraint: ToConstraint
    ) -> Constraint:  # TODO: define and use ToStepTypeHandler instead
        constraint = to_constraint(constraint)
        return Constraint(
...
class Circuit:
    ...
    def set_trace(
        self, trace_def: Callable[[TraceContext, Any], None]
    ):  # Any instead of TraceArgs
...
def to_expr(v: ToExpr) -> Expr:
    if isinstance(v, Expr):
        ...
        raise TypeError(
            f"Type {type(v)} is not ToExpr (one of Expr, str, int, F, Queriable, or Constraint)."
        )
...
def to_constraint(v: ToConstraint) -> Constraint:
    if isinstance(v, Constraint):
        ...
        raise TypeError(
            f"Type `{type(v)}` is not ToConstraint (one of Constraint, Expr, int, F, or Queriable)."
        )
